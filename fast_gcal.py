...
class CorticalMap(nn.Module):
    ...
    def __init__(        
        ...
        self.lat_tracker = torch.ones([lat_iters+lat_iters//2]+sheet_shape, device=device, dtype=self.type)
        self.last_lat = torch.zeros(sheet_shape, device=device, dtype=self.type)
        
        self.inh_spread = oddenise(sre_units*2)
        max_mask = get_circle(self.inh_spread,self.inh_spread/2).view(1,-1,1)
        self.max_mask = max_mask.to(device).type(self.type)
        self.prev_lat_tiles = torch.zeros((1, self.inh_spread**2, sheet_units**2), device=device, dtype=self.type)
        self.EBI_factors = torch.ones(sheet_shape, device=device, dtype=self.type)
        
        self.ebi_std = 17
        self.EBI_gaussian = get_gaussian(oddenise(self.ebi_std*5), self.ebi_std).to(device)
        self.EBI_gaussian = get_circle(rf_units, rf_units/2).to(device).repeat(1,2,1,1).view(1,-1,1)
    ...
    def forward(self, x, reco_flag=False, learning_flag=True):
        ...
        EBI_factors = EBI_factors.mean(1).view(lat.shape).view(lat.shape)
        ...
        EBI_factors = torch.exp(-EBI_factors*1e4) + 1e-7
        # reaction diffusion
        for i in range(self.lat_iters):
            ...
            differential = (lat_pos*self.EBI_factors - lat_neg)*self.strength
